...
private[spark] class EventLoggingListener(
  ...
  //var cpuTbl = new mutable.HashMap[String, Array[Float]]
  //var onStageCpu = new mutable.HashMap[String, Array[Float]]
  //var onCompCpu = new mutable.HashMap[String, Array[Float]]
  // Events that do not trigger a flush
  override def onStageSubmitted(event: SparkListenerStageSubmitted): Unit = {
    logEvent(event)
    logInfo("donglin: eventLogging is Enabled.")
    //cpuTbl = MetricGetter.GetExecutorCpuTable()
    //for((m,n) <- cpuTbl)
    //{
    //    onStageCpu(m) = n
        //logInfo(s"executor ${m} cpu usage is ${n(0)} + ${n(1)} + ${n(2)}")
    //}
  ...
  override def onStageCompleted(event: SparkListenerStageCompleted): Unit = {
    ...
    val onCompCpu = MetricGetter.GetExecutorCpuTable()
    val onStageCpu = MetricGetter.GetStageTable()
    for((k,v) <- onCompCpu)
    { 
        //logInfo(s"executor ${k} cpu usage is ${newVal(0)} + ${newVal(1)} + ${newVal(2)}")
        if (!onStageCpu.contains(k))
        {
        val cpuUsg = v(0)/(v(1)*v(2) *10000F)
        }
        else
        {
        val oldVal = onStageCpu(k)
        logInfo(s"executor ${k} cpu usage is ${oldVal(0)} + ${oldVal(1)} + ${oldVal(2)}")
        val cpuUsg = (v(0) - oldVal(0))/((v(1)*v(2) - oldVal(1)*oldVal(2))*10000F)
        logInfo(s"executor ${k} cpu usage is ${cpuUsg}")
        }
    }
    
    for((m,n) <- onCompCpu)
    {
        MetricGetter.UpdateStageTbl(m,n)
    }
