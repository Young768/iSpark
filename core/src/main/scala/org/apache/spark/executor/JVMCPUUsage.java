...
public class JVMCPUUsage {
    ...
    public float getJvmCpuUsage() {
        ...
        //float load = (float)peOperatingSystemMXBean.getProcessCpuLoad();
    ...
    public float[] getJvmCpuTime(){
    
        int size = 3;
        float[] cpuTime = new float[size];
        cpuTime[0] = peOperatingSystemMXBean.getProcessCpuTime();
        cpuTime[1] = runtimeMXBean.getUptime();
        cpuTime[2] = peOperatingSystemMXBean.getAvailableProcessors();
