...
private[spark] case object JVMHeapMaxMemory extends MetricGetter {
  override def getMetricValue(memoryManager: MemoryManager): Long = {
...
private[spark] case object JVMOffHeapMaxMemory extends MetricGetter {
  override def getMetricValue(memoryManager: MemoryManager): Long = {
...
private[spark] object MetricGetter {
  ...
  val cpuTimeArr = new mutable.HashMap[String, Array[Float]]
  
  def InitExecutorCpuTime(executorId: String): Unit = {
      val cput = new Array[Float](3)
      for (i <- 0 to (cput.length - 1)) {
         cput(i) = 0F
    } 
      cpuTimeArr(executorId) = cput
 }

      
  def UpdateExecutorCpuTime(executorId: String, cpuTime: Array[Float]): Unit = {
      if (!cpuTimeArr.contains(executorId)) {
        InitExecutorCpuTime(executorId)
      }

      cpuTimeArr(executorId)(0) = cpuTime(0)
      cpuTimeArr(executorId)(1) = cpuTime(1)
      cpuTimeArr(executorId)(2) = cpuTime(2)
  }

  def GetExecutorCpuTime(executorId: String): Array[Float] = {
    return cpuTimeArr(executorId)
  }
  
  def GetExecutorCpuTable(): mutable.HashMap[String,Array[Float]] = {
    return cpuTimeArr
  }

  val stageTbl = new mutable.HashMap[String, Array[Float]]

  def UpdateStageTbl(executorId: String, cpuTime: Array[Float]): Unit = {
      if (!stageTbl.contains(executorId)) {
         InitStageTbl(executorId)
      } 
         stageTbl(executorId)(0) = cpuTime(0)
         stageTbl(executorId)(1) = cpuTime(1)
         stageTbl(executorId)(2) = cpuTime(2)
  }

  def InitStageTbl(executorId: String): Unit = {
      val cput = new Array[Float](3)
      for (i <- 0 to (cput.length - 1)) {
             cput(i) = 0F
      }
      stageTbl(executorId) = cput
      }


  def GetStageTable(): mutable.HashMap[String,Array[Float]] = {
      return stageTbl
  }
