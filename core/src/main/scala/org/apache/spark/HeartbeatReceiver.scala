  ...
  override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
    ...
    case heartbeat @ Heartbeat(executorId, accumUpdates, blockManagerId, executorMetrics, cpuUsage, cpuTime) =>
      if (printTimes < 100000) {
        ...
        var temp = 0.0 
        temp = (executorMetrics(0) + executorMetrics(1)).toDouble/(executorMetrics(2) + executorMetrics(3)).toDouble
        logInfo(s"memoryusage is ${temp}.")

        MetricGetter.UpdateExecutorCpuUsage(executorId, cpuUsage)

        MetricGetter.UpdateExecutorCpuTime(executorId, cpuTime)
